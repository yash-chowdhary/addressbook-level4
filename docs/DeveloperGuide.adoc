...
image::EmailCommandUML.png[width="800"]
...
image::SDforEmailCommandParser.png[width="800"]
...
==== Current Implementation
The task-management mechanism is facilitated by several command classes in Club Connect, namely: +
...
All of these commands require a member to be logged in (and thus by extension, an initial sign-up).
Hence, they all share the following lines of code in their respective implementations:
...
Figure 23 shows the flow of parsing an `AddTaskCommand` object:

image::SDforAddTaskCommandParser.png[width="800"]
_Figure 23. Sequence Diagram for `AddTaskCommand` parsing._

The execution of the command invokes `Model#addTaskToTaskList()`. +
...
Figure 24 depicts the high-level sequence of events that take place.

image::SDforAddTask.png[width="800"]
_Figure 24. Sequence Diagram for adding a task._
...
Figure 25 shows the flow of parsing an `AssignTaskCommand` object:

image::SDforAssignTaskCommandParser.png[width="800"]
_Figure 25. Sequence Diagram for `AssignTaskCommand` parsing._

The execution of the command invokes `Model#assignTask()`.
The following code snippet shows the implementation:

[source,java]
@Override
public void assignTask(Task toAdd, MatricNumber matricNumber) throws MemberNotFoundException, DuplicateTaskException {
...
Figure 26 depicts the high-level sequence of events that take place.

image::SDforAssignTask.png[width="800"]
_Figure 26. Sequence Diagram for assigning a task._
...
Figure 27 illustrates the sequence in which the parsing is done.

image::SDforDeleteTaskCommandParser.png[width="800"]
_Figure 27. Sequence Diagram for `DeleteTaskCommand` parsing._

The execution of the command invokes `Model#deleteTask()`.
The following code snippet shows the implementation:
...
The high-level sequence of events is shown below in Figure 28.

image::SDforDeleteTask.png[width="800"]
_Figure 28. Sequence Diagram for deleting a task._
...
Figure 29 below depicts the parsing of the `ChangeTaskStatusObject`.

image::SDforChangeTaskStatusCommandParser.png[width]"800" ]
_Figure 29. Sequence Diagram for `ChangeTaskStatusCommand` parsing._


The execution of the command invokes `Model#changeStatus()`.
The following code snippet shows the implementation:
...
Figure 30 below describes the high-level sequence of events that take place.

image::SDforChangeTaskStatus.png[width="800"]
_Figure 30. Sequence Diagram for changing the status of a task._
...
The implementation makes use of a `Predicate` and is as follows:
...
The high-level sequence of events on executing the `ViewAllTasksCommand` is described below in Figure 31.

image::SDforViewAllTasks.png[width="800"]
_Figure 31. Sequence Diagram for viewing all tasks._
...
`Model#viewMyTasks()` is called during the execution of the command.
...
The sequence diagram of the command is shown below in Figure 32.

image::SDforViewMyTasks.png[width="800"]
_Figure 32. Sequence Diagram for viewing tasks of the currently logged-in member._
...
Figure 33 below shows the UML diagram of the `Poll` class.

image::PollUML.png[width="800"]
_Figure 33. Poll UML Diagram_
...
Figure 34 below shows the UML diagram of the `Answer` class.

image::AnswerUML.png[width="100"]
_Figure 34. Poll UML Diagram_
...
Figure 35 shows the flow of adding a poll:

image::SDforAddPoll.png[width="800"]
_Figure 35. Sequence Diagram for `AddPollCommand` parsing._
...
Figure 36 shows the flow of deleting a poll:

image::SDforDeletePoll.png[width="800"]
_Figure 36. Sequence Diagram for `DeletePollCommand` parsing._
...
Figure 37 shows the flow of voting in a poll:

image::SDforVote.png[width]"800" ]
_Figure 37. Sequence Diagram for voting a poll_
...
By default poll results are not shown(Figure 38), but when `ViewResultsCommand` is executed, results consisting of
how many people voted in a poll and how many voted for each answer will be shown(Figure 39).

image::PollWithoutResults.png[width="400"]
_Figure 38. Poll Without Results_

image::PollWithResults.png[width="400"]
_Figure 39. Poll With Results_

Since viewing and hiding results of polls are similar in implementation, only viewing results of polls will be discussed.

When an `Exco` member inputs 'viewresults', the sequence diagram (Figure 40) below shows how the different components interact
to post a `ViewResultsRequestEvent`.

image::HighLevelSDforViewResultsToEventsCenter.png[width="800"]
_Figure 40. Sequence Diagram to `EventsCenter` for viewing poll results ._

Then, the `PollListPanel` which contains the poll `ListView`, will handle the event (Figure 41) by showing results of results of all polls (Figure 39).

image::HighLevelSDforViewResultsFromEventsCenter.png[width="800"]
_Figure 41. Sequence Diagram from `EventsCenter` for viewing poll results._
...
The `DeleteGroupCommand` extends from `UndoableCommand` as it is an undoable command. Figure 42 below depicts the UML representation of the `Command`.

image::DeleteGroupCommandUML.png[width="200" height="200"]
_Figure 42. UML Diagram of `DeleteGroupCommand`._

`DeleteGroupCommandParser` is responsible for parsing the `deletegroup` command. It returns a `DeleteGroupCommand` object after parsing the `Group`. The parsing of the command is shown below in Figure 43.

image::SDforDeleteGroupCommandParser.png[width="800"]
_Figure 43. Sequence Diagram for the parsing of `DeleteGroupCommand`._

The execution invokes `Model#deleteGroup()` with the `Group` ```toRemove``` as a parameter. This further invokes the method `ClubBook#deleteGroup()`. Here is the implementation:
...
`ClubBook#deleteGroup()` further calls `ClubBook#deleteGroupFromClubBook()`.
...
The final method called is `ClubBook#deleteGroupFromMember()` which loops through all members and sets the group of all members that have the group ```toRemove```, to `member`.
...
The high-level sequence of events is depicted in Figure 44 below.

image::SDforDeleteGroup.png[width="800"]
_Figure 44. Sequence Diagram for delete a group from `Club Connect`._
...
Refer to Figure 45 for the UML diagram.
...
_Figure 45. ChangeProfilePhotoCommand UML Diagram_

The `displaypic` command involves the use of multiple components of Club Connect.
Below, Figure 46 shows the interactions betweeen these components.
...
_Figure 46. High Level Sequence Diagram for `changepic PATH` Command_
...
Figure 47 depicts the Sequence Diagram for interactions within the `Logic` component for the `execute("changepic C:/Users/Admin/Desktop/ photo.png")` API call.

image::ChangeProfilePhotoCommandSdForLogic.png[width="650"]
_Figure 47. Interactions Inside the Logic Component for the `changepic C:/Users/Admin/Desktop/ photo.png` Command_
...
Figure 48 (shown below) depicts the UML representation of the `DeleteTagCommand`.

image::placeholder.png[width="200" height="200"]
_Figure 48. UML Diagram of `DeleteTagCommand`._
...
==== Current Implementation
Member details are shown as cards in the GUI. Cards can either be decompressed(Figure 49) or compressed(Figure 50).

image::DecompressedMemberCards.png[width="300"]
_Figure 49. Decompressed Member Cards_

image::CompressedCards.png[width="300"]
_Figure 50. Compressed Member Cards_
...
To differentiate between compressed and decompressed member card, we introduced a `CompressedMemberCard` class. This class extends `MemberCard` (Figure 51) since a `CompressedMemberCard` is a `MemberCard`.
...
_Figure 51. `MemberCard` class diagram_
...
Suppose that the user has just launched the application. The member cards would be decompressed (Figure 50).

The user inputs 'compress' and the sequence diagram (Figure 52) below shows how the different components interact.

image::SDforCompressToEventsCenter.png[width="800"]
_Figure 52. Sequence diagram for compress command to Events Center_

Then, the `MemberListPanel`, which contains the `MemberCard` objects will handle the event (Figure 53). If the cards are already compressed, no changes occur.
...
_Figure 53. Sequence diagram for compress command from Events Center_
...
.  Set the destination to `Save as PDF`, then click `Save` to save a copy of the file in PDF format. For best results, use the settings in Figure 54 below.

image::chrome_save_as_pdf.png[width="300"]
_Figure 54. Screenshot of saving documentation as PDF in Chrome_
...
. Use different colors for different tags inside member cards. For example, `friends` tags can be all in brown, and `colleagues` tags can be all in yellow as shown in Figure 46.
+
**Before**
+
image::getting-started-ui-tag-before.png[width="300"]
_Figure 55. Member details before modification_
+
**After**
+
image::getting-started-ui-tag-after.png[width="300"]
_Figure 56. Member details after modification_
+
****
* Hints
...
+
Figure 57 below shows the `ResultDisplay` before the modification. +
**Before**
+
image::getting-started-ui-result-before.png[width="200"]
_Figure 58. ResultDisplay before modification_
+
**After**
+
Figure 59 below shows the `ResultDisplay` after the modification.
+
image::getting-started-ui-result-after.png[width="200"]
_Figure 60. ResultDisplay after modification_
+
****
* Hints
...
+
Figure 61 below shows the `StatusBarFooter` before the modification.
+
**Before**
+
image::getting-started-ui-status-before.png[width="500"]
_Figure 62. StatusBarFooter before modification_
+
Figure 63 below shows the `StatusBarFooter` after the modification.
+
**After**
+
image::getting-started-ui-status-after.png[width="500"]
_Figure 64. StatusBarFooter after modification_
+
****
* Hints
