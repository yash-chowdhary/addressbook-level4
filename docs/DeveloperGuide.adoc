...
// tag::removegroup[]
=== Remove Group Mechanism
==== Current Implementation

The `removegroup` mechanism is facilitated by the `RemoveGroupCommand` class. It allows `Exco` members to remove a group from `Club Connect`. The group of all members part of the group that is to be deleted will be changed to the default group - `member`.

The `RemoveGroupCommand` extends from `UndoableCommand` as it is an undoable command. Figure 33 below depicts the UML representation of the `Command`.

image::RemoveGroupCommandUML.png[width="200" height="200"]
_Figure 33. UML Diagram of `RemoveGroupCommand`._

`RemoveGroupCommandParser` is responsible for parsing the `removegroup` command. It returns a `RemoveGroupCommand` object after parsing the `Group`. The parsing of the command is shown below in Figure 34.

image::SDforRemoveGroupCommandParser.png[width="800"]
_Figure 34. Sequence Diagram for the parsing of `RemoveGroupCommand`._

The execution invokes `Model#removeGroup()` with the `Group` ```toDelete``` as a parameter. This further invokes the method `ClubBook#removeGroup()`. Here is the implementation:
...
`ClubBook#removeGroup()` further calls `ClubBook#removeGroupFromClubBook()`.
...
The final method called is `ClubBook#removeGroupFromMember()` which loops through all members and sets the group of all members that have the group ```toDelete``` to `member`.
...
_Figure 35. Sequence Diagram for removing a group from `Club Connect`._


==== Design Considerations
===== Aspect: Implementation of `RemoveGroupCommand`
...
* Minor Feature: Find Command
+
Improved Find Command such that users can search members by a specified field which includes: Name, Phone, Email, Matric Number, Tags and Group. This increases navigability of the contacts inside Club Connect.
