...
are allowed to add/delete polls and view/hide poll results.
...
`ViewResultsCommand` and `HideResultsCommand` will have the statement below in their `execute` method.

[source, java]
requireToSignUp();

`VoteCommand` will have the statement below in it's execute method since it doesn't require `Exco` priviledge but still
require that the user is logged in to keep track of who is voting.

[source, java]
requireToLogIn();

Figure 22 below shows the UML diagram of the `Poll` class.

image::TaskUML.png[width="800"]
_Figure 22. Task UML Diagram_

A Poll object consists of the following:
...
===== Adding Polls
---
Adding Polls is facilitated by the `AddPollCommand`. When a poll is created by the `AddPollCommand`, it's Set<MatricNumber>
is initialized as empty to show that no member has voted for the poll.

Figure 23 shows the flow of adding a poll:

image::SDforAddTaskCommandParser.png[width="800"]
_Figure 23. Sequence Diagram for `AddPollCommand` parsing._
...
image::SDforAddTask.png[width="800"]
_Figure 24. Sequence Diagram for adding a poll._

The poll will be displayed in the poll list.

===== Deleting Polls
---
Deleting Polls is facilitated by the `DeletePollCommand`.

Figure 23 shows the flow of deleting a poll:

image::SDforDeleteTaskCommandParser.png[width="800"]
_Figure 27. Sequence Diagram for `DeletePollCommand` parsing._
...
image::SDforDeleteTask.png[width="800"]
_Figure 28. Sequence Diagram for deleting a poll._


===== Voting in Polls
---

Deleting Polls is facilitated by the `VoteCommand`.

The input of the command is the `POLL_INDEX` of a poll and `ANSWER_INDEX` of the answer of the poll

Figure 23 shows the flow of voting in a poll:

image::SDforChangeTaskStatusCommandParser.png[width]"800" ]
_Figure 29. Sequence Diagram for voting a poll_
...
** Pros: Easy to implement
** Cons: If data of a poll in club book file is corrupted, the sum of votes for all the poll's answers might not be
equal to the size of the Set<MatricNumber> of the poll.
...
The `DeleteGroupCommand` extends from `UndoableCommand` as it is an undoable command. Figure 33 below depicts the UML representation of the `Command`.

image::DeleteGroupCommandUML.png[width="200" height="200"]
_Figure 33. UML Diagram of `DeleteGroupCommand`._

`DeleteGroupCommandParser` is responsible for parsing the `deletegroup` command. It returns a `DeleteGroupCommand` object after parsing the `Group`. The parsing of the command is shown below in Figure 34.

image::SDforDeleteGroupCommandParser.png[width="800"]
_Figure 34. Sequence Diagram for the parsing of `DeleteGroupCommand`._
...
The high-level sequence of events is depicted in Figure 35 below.

image::SDforDeleteGroup.png[width="800"]
_Figure 35. Sequence Diagram for delete a group from `Club Connect`._
...
Below, Figure 36 shows the interactions betweeen these components.
...
_Figure 36. High Level Sequence Diagram for `changepic PATH` Command_
...
Figure 37 depicts the Sequence Diagram for interactions within the `Logic` component for the `execute("changepic C:/Users/Admin/Desktop/ photo.png")` API call.

image::ChangeProfilePhotoCommandSdForLogic.png[width="650"]
_Figure 37. Interactions Inside the Logic Component for the `changepic C:/Users/Admin/Desktop/ photo.png` Command_
...
Figure 38 (shown below) depicts the UML representation of the `DeleteTagCommand`.

image::placeholder.png[width="200" height="200"]
_Figure 38. UML Diagram of `DeleteTagCommand`._
...
==== Current Implementation
Member details are shown as cards in the GUI. Cards can either be decompressed(Figure 39) or compressed(Figure 40).

image::DecompressedMemberCards.png[width="300"]
_Figure 39. Decompressed Member Cards_

image::CompressedCards.png[width="300"]
_Figure 40. Compressed Member Cards_
...
To differentiate between compressed and decompressed member card, we introduced a `CompressedMemberCard` class. This class extends `MemberCard` (Figure 41) since a `CompressedMemberCard` is a `MemberCard`.
...
_Figure 41. `MemberCard` class diagram_
...
The user inputs 'compress' and the sequence diagram (Figure 42) below shows how the different components interact.

image::SDforCompressToEventsCenter.png[width="800"]
_Figure 42. Sequence diagram for compress command to Events Center_

Then, the `MemberListPanel`, which contains the `MemberCard` objects will handle the event (Figure 43). If the cards are already compressed, no changes occur.
...
_Figure 43. Sequence diagram for compress command from Events Center_
...
.  Set the destination to `Save as PDF`, then click `Save` to save a copy of the file in PDF format. For best results, use the settings in Figure 44 below.

image::chrome_save_as_pdf.png[width="300"]
_Figure 44. Screenshot of saving documentation as PDF in Chrome_
...
_Figure 45. Member details before modification_
+
**After**
+
image::getting-started-ui-tag-after.png[width="300"]
_Figure 46. Member details after modification_
+
****
* Hints
...
+
Figure 47 below shows the `ResultDisplay` before the modification. +
**Before**
+
image::getting-started-ui-result-before.png[width="200"]
_Figure 47. ResultDisplay before modification_
+
**After**
+
Figure 48 below shows the `ResultDisplay` after the modification.
+
image::getting-started-ui-result-after.png[width="200"]
_Figure 48. ResultDisplay after modification_
+
****
* Hints
...
+
Figure 49 below shows the `StatusBarFooter` before the modification.
+
**Before**
+
image::getting-started-ui-status-before.png[width="500"]
_Figure 49. StatusBarFooter before modification_
+
Figure 50 below shows the `StatusBarFooter` after the modification.
+
**After**
+
image::getting-started-ui-status-after.png[width="500"]
_Figure 50. StatusBarFooter after modification_
+
****
* Hints
