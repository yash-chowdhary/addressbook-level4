...
The `RemoveGroupCommand` extends from `UndoableCommand` as it is an undoable command. Figure 40 below depicts the UML representation of the `Command`.

image::RemoveGroupCommandUML.png[width="200" height="200"]
_Figure 40. UML Diagram of `RemoveGroupCommand`._

`RemoveGroupCommandParser` is responsible for parsing the `removegroup` command. It returns a `RemoveGroupCommand` object after parsing the `Group`. The parsing of the command is shown below in Figure 41.

image::SDforRemoveGroupCommandParser.png[width="800"]
_Figure 41. Sequence Diagram for the parsing of `RemoveGroupCommand`._
...
_Figure 42. Sequence Diagram for removing a group from `Club Connect`._
