...
*Club Connect* is a computer application targeted at <<DeveloperGuide#club,club>> <<DeveloperGuide#member,members>> who are students at the National University of Singapore (NUS).
It aims to make the tedious process of club management easier and more effective.

*Club Connect* is an application that would be preferred by those who prefer using the keyboard over a mouse, i.e., it uses a Command Line Interface (CLI).
All output is displayed in a Graphical User Interface (GUI) - which is just computer jargon for a display that includes panes, menus and message boxes.
...
include::../DeveloperGuide.adoc[tag=ap-manual-testing]
