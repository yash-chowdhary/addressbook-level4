...
image::Ui.png[width="690"][align="left"]
+
...
image::help-window.png[align="center"][width="400"]
+
...
* Items with `…`​ after them can be used multiple times including zero times e.g. `[t/TAG]...` can be used as `{nbsp}` (i.e. 0 times), `t/Treasurer`, `t/Operations t/Logistics` etc.
...
A person will be assigned to the `member` group by default.
[TIP]
...
Edits an existing member in Club Connect. +
...
Edits the phone number and email address of the 1st member to `91234567` and `johndoe@example.com` respectively.
