...
* Club Connect detects if there is no change in the `ASSIGNEE`.
* Club Connect also detects if the command results in a duplicate task in the task list.
* You cannot change the `ASSIGNEE` to a member who has already been assigned a similar task by another `Exco` member.
****
...
[NOTE]
You will face a longer delay when setting your profile photo to an image whose file size is above 500 KB.
